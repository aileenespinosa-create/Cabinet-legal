...
type Lang = "ES" | "EN" | "FR";
type Page = "home" | "firma" | "socios" | "servicios" | "consulta";
...
function PortraitFallback({ label }: { label: string }) {
  return (
    <div className="mx-auto flex w-full max-w-[320px] aspect-[4/5] items-center justify-center rounded-[28px] border border-dashed border-slate-300 bg-slate-50 px-6 text-center text-sm text-slate-400">
      {label}
    </div>
  );
}
          ...
          <div className="grid gap-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="rounded-[28px] bg-[#0A3A5A] p-6 text-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
                ...
                <div className="mt-3 text-2xl font-serif">{t.servicesHighlightTitle}</div>
                <p className="mt-3 text-sm leading-7 text-slate-200">{t.servicesHighlightText}</p>
              </div>
    ...
    <section className="bg-white py-24 md:py-32">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid gap-14 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div>
            <div className="text-xs uppercase tracking-[0.28em] text-[#D9BE3F]">Cabinet Legal</div>
            <h2 className="mb-6 mt-4 text-4xl font-serif font-semibold text-[#0A3A5A]">{t.firma}</h2>
      ...
      <div className="mx-auto grid max-w-7xl gap-16 px-6 md:grid-cols-2">
        <div className="group text-center">
          <ImageWithFallback
            src={AILEEN_IMAGE}
            alt="Aileen Espinosa"
            className="mx-auto w-full max-w-[320px] aspect-[4/5] object-cover rounded-[28px] grayscale transition duration-500 group-hover:grayscale-0 shadow-lg"
    ...
    <section className="bg-white py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6">
        <div className="mb-16 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-xs uppercase tracking-[0.28em] text-[#D9BE3F]">Cabinet Legal</div>
            <h2 className="mt-4 text-4xl font-serif font-semibold text-[#0A3A5A]">{t.servicios}</h2>
          </div>
        ...
        <div className="grid gap-8 md:grid-cols-3">
          {[
            ...
            <div key={title} className="group rounded-2xl border border-slate-200 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
              <div className="mb-4 h-1 w-12 rounded-full transition-all duration-300 group-hover:w-20" style={{ backgroundColor: GOLD }} />
  ...
  const renderConsulta = () => (
    <section className="bg-white py-24 md:py-32">
      <div className="mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[0.85fr_1.15fr]">
        <div>
          <div className="text-xs uppercase tracking-[0.28em] text-[#D9BE3F]">Cabinet Legal</div>
          <h2 className="mt-4 text-4xl font-serif font-semibold text-[#0A3A5A]">{t.consultationTitle}</h2>
        ...
        <div className="rounded-[32px] border border-slate-200 bg-[#FBFBF8] p-8 shadow-sm">
          {formSubmitted ? (
            <div className="flex min-h-[540px] flex-col items-center justify-center text-center">
              <div className="text-xs uppercase tracking-[0.28em] text-[#D9BE3F]">Cabinet Legal</div>
              <h3 className="mt-4 text-3xl font-serif font-semibold text-[#0A3A5A]">{t.consultationTitle}</h3>
              <p className="mt-4 max-w-md text-sm leading-7 text-slate-600">{t.consultationSuccess}</p>
              <button
            ...
            <form className="grid gap-6" onSubmit={handleConsultationSubmit}>
              <div className="grid gap-6 md:grid-cols-2">
                <label className="grid gap-2 text-sm text-slate-600">
                  ...
                  <input name="nombre" className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-[#0A3A5A]" type="text" required />
                </label>
                <label className="grid gap-2 text-sm text-slate-600">
                  <span>{t.emailLabel}</span>
                  <input name="email" className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-[#0A3A5A]" type="email" required />
                </label>
              </div>
              <div className="grid gap-6 md:grid-cols-2">
                <label className="grid gap-2 text-sm text-slate-600">
                  <span>{t.phoneLabel}</span>
                  <input name="telefono" className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-[#0A3A5A]" type="tel" />
                </label>
                <label className="grid gap-2 text-sm text-slate-600">
                  <span>{t.companyLabel}</span>
                  <input name="empresa" className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-[#0A3A5A]" type="text" />
                </label>
              </div>
              <label className="grid gap-2 text-sm text-slate-600">
                <span>{t.matterLabel}</span>
                <input name="asunto" className="rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-[#0A3A5A]" type="text" required />
              </label>
              <label className="grid gap-2 text-sm text-slate-600">
                <span>{t.messageLabel}</span>
                <textarea name="mensaje" className="min-h-[180px] rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none transition focus:border-[#0A3A5A]" required />
              </label>
    ...
    <main className="min-h-screen bg-white">
      ...
      <footer className="border-t border-slate-200 bg-white py-10">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-sm font-semibold tracking-[0.28em] text-[#0A3A5A]">CABINET LEGAL</div>
            <div className="mt-2 text-xs uppercase tracking-[0.22em] text-slate-400">Attorneys at law</div>
          </div>
          <div className="text-center text-xs tracking-[0.2em] text-slate-500 md:text-right">© 2026 Cabinet Legal</div>
        </div>
      </footer>
    </main>
  );
